...
if (isset($_POST['save'])) {
    ...
    if (empty($nombre)) {
        echo "<script>
                Swal.fire('Advertencia', 'Por favor, completa todos los campos antes de enviar el formulario.', 'warning').then(function() {
                    window.location.href = '../view_rte.php';
                });
              </script>";
        exit();
    }
    ...
    $imagen_ruta = "../assets/documentacion/" . $imagen_nombre;
    move_uploaded_file($imagen_tmp, $imagen_ruta);

    // Insertar datos en la base de datos
    $sql = "INSERT INTO rte_section (nombre_rte, archivo_rte)
            VALUES ('$nombre', '$imagen_ruta')";

    if ($con->query($sql) === TRUE) {
        echo "<script>
                Swal.fire('Éxito', 'Registro de RTE exitoso.', 'success').then(function() {
                    window.location.href = '../view_rte.php';
                });
              </script>";
    } else {
        echo "<script>
                Swal.fire('Error', 'Error al registrar RTE: " . $con->error . "', 'error').then(function() {
                    window.location.href = '../view_rte.php';
                });
              </script>";
    }
    ...
    if ($_FILES['archivo']['error'] === UPLOAD_ERR_OK) {
        ...
        $sql = "UPDATE rte_section SET
                nombre_rte = '$nombre',
                archivo_rte = '$imagen_ruta'
                WHERE id = $edit_id";
    ...
    if ($con->query($sql) === TRUE) {
        echo "<script>
                    Swal.fire('Éxito', 'Actualización de RTE exitosa', 'success').then(function() {
                        window.location.href = '../view_rte.php';
                    });
                </script>";
    } else {
        echo "<script>
                    Swal.fire('Error', 'Error al actualizar RTE: " . $con->error . "', 'error').then(function() {
                        window.location.href = '../view_rte.php';
                    });
                </script>";
    }
    ...
    $sql = "DELETE FROM rte_section WHERE id = $delete_id";
